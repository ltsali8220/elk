...
def build():
    ...
    S += [note("<b>Navigation:</b>  Analytics -> Dashboards -> Create dashboard -> Create visualization"),
          SP(8),

          # ── How to add any panel ────────────────────────────────────────────
          h3("How to add any panel"),
          tbl(["Step","Action"],
              [["1","Analytics -> Dashboards -> Create dashboard"],
               ["2","Click  Create visualization"],
               ["3","Select the Data view from the top-left dropdown"],
               ["4","Choose chart type from the right-side panel"],
               ["5","Drag fields onto Horizontal axis / Vertical axis / Slice by"],
               ["6","Type KQL filter in the search bar at the top (if needed)"],
               ["7","Click Apply  ->  Save and return"],
               ["8","Repeat for each panel, then click Save dashboard"]],
              [8*mm, TW-8*mm]),
          SP(10), PageBreak(),

          # ── Dashboard 1 ─────────────────────────────────────────────────────
          *sec("10a.  Dashboard 1 — Auth & Security  (auth-logs-*)"),
          body("Create dashboard, name it  <b>Auth &amp; Security</b>,  data view: <b>auth-logs-*</b>"),
          SP(6),
          h3("Panel 1  —  Login Timeline"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"],
               ["KQL filter",      "tags: linux_auth"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 2  —  SSH Success vs Failure"),
          tbl(["Setting","Value"],
              [["Chart type",  "Donut"],
               ["Slice by",    "Top values of tags  (size 5)"],
               ["Metric",      "Count of records"],
               ["KQL filter",  "tags: ssh_login_success OR tags: ssh_login_failure"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 3  —  Top Failed IPs"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of client.ip  (size 10)"],
               ["Horizontal axis", "Count of records"],
               ["KQL filter",      "tags: ssh_login_failure"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 4  —  sudo Commands"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of message  (size 10)"],
               ["Horizontal axis", "Count of records"],
               ["KQL filter",      "tags: sudo_command"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 5  —  Failed Login Count  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type",  "Metric"],
              ...
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 6  —  Login Success Count  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type",  "Metric"],
               ["Value",       "Count of records"],
               ["KQL filter",  "tags: ssh_login_success"]],
              [40*mm, TW-40*mm]),
          SP(4), PageBreak(),

          # ── Dashboard 2 ─────────────────────────────────────────────────────
          *sec("10b.  Dashboard 2 — System Overview  (syslog-*)"),
          body("Create dashboard, name it  <b>System Overview</b>,  data view: <b>syslog-*</b>"),
          SP(6),
          h3("Panel 1  —  Log Volume Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"],
              ...
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 2  —  Top Processes"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of process.name  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 3  —  Kernel Events Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"],
               ["KQL filter",      "tags: kernel_event"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 4  —  Error Count  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type",  "Metric"],
               ["Value",       "Count of records"],
               ["KQL filter",  "message: *error*"]],
              [40*mm, TW-40*mm]),
          SP(4), PageBreak(),

          # ── Dashboard 3 ─────────────────────────────────────────────────────
          *sec("10c.  Dashboard 3 — Docker Containers  (docker-logs-*)"),
          body("Create dashboard, name it  <b>Docker Containers</b>,  data view: <b>docker-logs-*</b>"),
          SP(6),
          h3("Panel 1  —  Logs per Container"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of container.name  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 2  —  Error Spike Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"],
               ["KQL filter",      "message: *error* OR message: *ERROR*"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 3  —  Log Volume Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"],
              ...
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 4  —  Total Container Events  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type", "Metric"],
               ["Value",      "Count of records"]],
              [40*mm, TW-40*mm]),
          ...
          # ── Dashboard 4 ─────────────────────────────────────────────────────
          *sec("10d.  Dashboard 4 — Journal & Services  (journal-logs-*)"),
          body("Create dashboard, name it  <b>Journal &amp; Services</b>,  data view: <b>journal-logs-*</b>"),
          SP(6),
          h3("Panel 1  —  Failed Services Count  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type",  "Metric"],
               ["Value",       "Count of records"],
               ["KQL filter",  'message: "failed"']],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 2  —  Top systemd Units"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of systemd.unit  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 3  —  Journal Events Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 4  —  Events by Unit (Donut)"),
          tbl(["Setting","Value"],
              [["Chart type", "Donut"],
               ["Slice by",   "Top values of systemd.unit  (size 8)"],
               ["Metric",     "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(4), PageBreak(),

          # ── Dashboard 5 ─────────────────────────────────────────────────────
          *sec("10e.  Dashboard 5 — App Logs  (app-logs-*)"),
          body("Create dashboard, name it  <b>App Logs</b>,  data view: <b>app-logs-*</b>"),
          SP(6),
          h3("Panel 1  —  Request Volume Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 2  —  Logs by Source"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of log.file.path  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 3  —  Error Events Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"],
              ...
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 4  —  Total App Events  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type", "Metric"],
               ["Value",      "Count of records"]],
              [40*mm, TW-40*mm]),
          ...
          # ── Dashboard 6 — SIEM Overview ──────────────────────────────────────
          *sec("10f.  Dashboard 6 — SIEM Overview  (all indices)"),
          body("Create dashboard, name it  <b>SIEM Overview</b>."
               "  Add panels using  <b>Add from library</b>  to reuse panels saved from other dashboards."),
          SP(6),
          tbl(["Panel","Data view","Chart","Field / KQL"],
              [["Total Auth Events",     "auth-logs-*",    "Metric",          "Count"],
               ["Total Syslog Events",   "syslog-*",       "Metric",          "Count"],
               ["Total Docker Events",   "docker-logs-*",  "Metric",          "Count"],
               ["Total Journal Events",  "journal-logs-*", "Metric",          "Count"],
               ["Total App Events",      "app-logs-*",     "Metric",          "Count"],
               ["Auth Events Timeline",  "auth-logs-*",    "Bar stacked",     "@timestamp"],
               ["Syslog Timeline",       "syslog-*",       "Bar stacked",     "@timestamp"],
               ["Failed Logins",         "auth-logs-*",    "Metric",          "tags: ssh_login_failure"],
               ["sudo Commands",         "auth-logs-*",    "Bar horizontal",  "Top values: message"]],
              [38*mm, 30*mm, 24*mm, TW-92*mm]),
          SP(8),
          note("<b>Tip — Add from library:</b>  When editing a dashboard, click  "
               "Add panel -> Add from library  to insert any visualization you already "
               "saved from another dashboard. This avoids rebuilding the same panel twice."),
          PageBreak()]

    # ── 10g. WINDOWS SECURITY ─────────────────────────────────────────────────
    S += sec("10g.  Dashboard 7 — Windows Security  (windows-logs-*)")
    S += [body("Create dashboard, name it  <b>Windows Security</b>,  data view: <b>windows-logs-*</b>"),
          SP(6),
          h3("Panel 1  —  Events Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
               ["Horizontal axis", "@timestamp"],
               ["Vertical axis",   "Count of records"],
              ...
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 2  —  Top Event IDs"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of winlog.event_id  (size 15)"],
               ["Horizontal axis", "Count of records"],
               ["KQL filter",      "(none)"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 3  —  Events by Channel"),
          tbl(["Setting","Value"],
              [["Chart type", "Donut"],
               ["Slice by",   "Top values of winlog.channel  (size 8)"],
               ["Metric",     "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 4  —  Events by Log Level"),
          tbl(["Setting","Value"],
              [["Chart type", "Donut"],
               ["Slice by",   "Top values of log.level  (size 6)"],
               ["Metric",     "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 5  —  Login Successes  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type",  "Metric"],
               ["Value",       "Count of records"],
               ["KQL filter",  "tags: login_success"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 6  —  Login Failures  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type",  "Metric"],
               ["Value",       "Count of records"],
               ["KQL filter",  "tags: login_failure"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 7  —  Top Event Providers"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of event.provider  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 8  —  Security Event Tags"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of tags  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(4),
          note("<b>Key Windows Event IDs:</b>  "
               "4624 = Login success  |  4625 = Login failure  |  "
               "4634/4647 = Logout  |  4688 = Process created  |  "
               "4698/4702 = Scheduled task  |  4720 = User account created"),
          SP(4), PageBreak()]

    # ── 10h. WEBAPP ACTIVITY ──────────────────────────────────────────────────
    S += sec("10h.  Dashboard 8 — Webapp Activity  (webapp-logs-*)")
    S += [body("Create dashboard, name it  <b>Webapp Activity</b>,  data view: <b>webapp-logs-*</b>"),
          SP(6),
          h3("Panel 1  —  Request Volume Over Time"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar vertical stacked"],
              ...
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 2  —  Log Type Breakdown"),
          tbl(["Setting","Value"],
              [["Chart type", "Donut"],
               ["Slice by",   "Top values of fields.log_subtype  (size 5)"],
               ["Metric",     "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 3  —  Tags Breakdown"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of tags  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 4  —  Total Log Events  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type", "Metric"],
               ["Value",      "Count of records"],
               ["KQL filter", "(none)"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 5  —  Login Events  (Metric)"),
          tbl(["Setting","Value"],
              [["Chart type",  "Metric"],
               ["Value",       "Count of records"],
               ["KQL filter",  "tags: webapp AND message: *login*"]],
              [40*mm, TW-40*mm]),
          SP(6),
          h3("Panel 6  —  Events by Agent"),
          tbl(["Setting","Value"],
              [["Chart type",      "Bar horizontal"],
               ["Vertical axis",   "Top values of agent.name  (size 10)"],
               ["Horizontal axis", "Count of records"]],
              [40*mm, TW-40*mm]),
          SP(4), PageBreak()]
